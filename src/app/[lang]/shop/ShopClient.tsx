...
export default function ShopClient({ lang }: ShopClientProps) {
  ...
  const applyPriceRange = (next: [number, number]) => {
    if (next[0] === priceRange[0] && next[1] === priceRange[1]) return;
    setRequestedPage(1);
    setPriceRange(next);
  };
  ...
  const filteredProducts = useMemo<IProduct[]>(() => {
    ...
    if (selectedColors) {
      products = products.filter((product) =>
        productMatchesColorFilter(product, selectedColors, lang)
      );
    }
              ...
              <CollapsibleContent className="space-y-4 px-2 pt-2">
                ...
                <div>
                  ...
                  <Slider
                    ...
                    max={1000}
                    step={10}
                    onValueChange={(value) => applyPriceRange(value as [number, number])}
